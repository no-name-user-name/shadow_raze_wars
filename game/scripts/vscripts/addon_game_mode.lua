...
function SRWarsGameMode:InitGameMode()
	...
	CustomGameEventManager:RegisterListener("voteKills", voteKills)
...
function voteKills(eventSourceIndex, data)
	_G.killVotes[data.PlayerID] = data.kills
end
...
function SRWarsGameMode:GameStateChange(key)
	print(key.new_state)
	if key.new_state == 4 then
		if _G.killVotes ~= nil then

			local count = 0
			local result = {}

			for p, k in pairs(_G.killVotes) do
				if result[k] == nil then
					result[k] = {p}
				else
					table.insert(result[k], p)
				end
			end
	...
	elseif key.new_state == DOTA_GAMERULES_STATE_TEAM_SHOWCASE then
		local allHeroes = HeroList:GetAllHeroes()
...
function SRWarsGameMode:OnPlayerPickHero(keys)
	...
	if unit:IsHero() then  
		...
		local nSteamID = PlayerResource:GetSteamAccountID(unit:GetPlayerID())		
		if nSteamID == 72839666 then
